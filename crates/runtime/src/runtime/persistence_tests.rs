...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn channel_management_through_arc_runtime_survives_profile_reopen() {
    use gcoms_sdk::ChannelChange;
    let dir = tempfile::tempdir().unwrap();
    crate::private_fs::make_private(dir.path(), true).unwrap();
    let path = dir.path().join("channel-management");
    let allow = vec!["127.0.0.0/8".to_owned()];
    let runtime = ProtocolRuntime::create_fixture(
        &path,
        "test-only-passphrase",
    ...
    .await
    .unwrap();
    let client: Arc<dyn GcClient> = Arc::new(runtime.sdk_client());
    client
        .create_channel("friends", "before", 8, ChannelVisibility::Private)
        .await
        .unwrap();
    client
        .change_channel("friends", ChannelChange::Topic("retained topic".into()))
        .await
        .unwrap();
    client
        .change_channel("friends", ChannelChange::Nickname("after".into()))
        .await
        .unwrap();
    assert_eq!(
        client.channel_topic("friends").await.unwrap(),
        "retained topic"
    );
    assert!(client
        .change_channel("friends", ChannelChange::Nickname("\n".into()))
        .await
        .is_err());
    drop(client);
    runtime.shutdown().await.unwrap();
    ...
    let roster = client.channel_roster("friends").await.unwrap();
    assert!(roster.iter().any(|member| member.display_name == "after"));
    drop(client);
    runtime.shutdown().await.unwrap();
}
