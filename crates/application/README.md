...
Default features include `embedded`, `ipc`, and typed `rpc`. An IPC-only host can
use `default-features = false, features = ["ipc"]`; portable service definitions
use no default features and browser clients add `wasm`.

## Shared service

Only the backend selection changes:
...
The operated GChat network's public configuration is bundled. First use needs a
...
management operations require IPC17; historical request tags remain stable.
