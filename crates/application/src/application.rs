use crate::{control, rpc, sdk};
use rpc::gc::{DirectLink, GcEndpoint, Peer};
...
impl ApplicationBuilder {
    ...
    pub async fn open(self) -> Result<Application, String> {
        ...
        let opened = async {
            ...
            let handles = Arc::new(
                rpc::file_store::FileHandles::open(&control::sidecar(&profile, "gcoms-handles"))
                    .map_err(|e| e.to_string())?,
            );
            ...
            Ok::<_, String>((endpoint, peers, handles, messages, task, errors_rx))
        }
        .await;
        match opened {
            Ok((endpoint, peers, handles, messages, task, errors)) => {
                Ok(Application(Arc::new(Inner {
                    sdk,
                    backing,
                    endpoint,
                    peers,
...
impl Application {
    ...
    pub fn rpc(
        ...
        let endpoint = self.0.endpoint.as_ref().ok_or_else(|| {
        ...
        .with_handles(self.0.handles.clone()))
    }
    pub fn handles(&self) -> &dyn rpc::HandleStore {
        self.0.handles.as_ref()
    }
    ...
    async fn close_inner(&self, stop: bool) -> Result<(), String> {
        if !self.0.closed.swap(true, Ordering::AcqRel) {
            ...
            let stopped = if let Some(endpoint) = &self.0.endpoint {
        ...
        tokio::select! {
            ...
            _ = tokio::time::sleep(Duration::from_millis(50)) => {
                ...
                for delivery in deliveries {
                    ...
                    let sdk = sdk.clone(); let endpoint = endpoint.clone(); let sender = sender.clone(); let errors = errors.clone();
